...
namespace DoctypeEncodingValidation
{
    ...
    public class HTMLValidate
    {
        ...
        /// </summary>
        private Faults faults; //我修改了访问权限
        /// <summary>
        ...
        #region 获取 错误 error waring 
        /// <summary>
        /// 获取检测的所有结果 错误 警告
        /// 注意要先 进行参数的初始化
        /// 发布到平台才能调用w3school的 服务
        /// </summary>
        /// <param name="htmlValidate"></param>
        /// <returns></returns>
        public string GetaCheckResult(HTMLValidate htmlValidate)
        {
            string strResult = "";
            ...
            try
            {
                ...
                {
                    strResult += "Invalid Markup validation of the document \r\n";
                }
                else
                {
                    strResult += "Valid Markup validation of the document \r\n";
                }
            }
            catch (ControlException ex)
            {
                strResult += ex.Message + "    " + ex.errorTimeStamp.ToString();
            }
            ...
            try
            {
                ...
                {
                    strResult += "This document is NOT a valid utf-8 document \r\n";
                    ...
                    strResult += "This document is a valid utf-8 document \r\n";
            }
            catch (ControlException ex)
            {
                strResult += ex.Message + "    " + ex.errorTimeStamp.ToString();
            ...
            try
            {
                ...
                {
                    strResult += "This document is NOT a valid html document \r\n";
                    ...
                    foreach (WarningPotentialIssue warningPotentialIssue in htmlValidate.warningPotentialIssues)
                    {
                        ...
                        strResult += warningPotentialIssue.ToString() + "\r\n";
                    }

                    foreach (Warning warning in htmlValidate.warnings)
                    {
                        strResult += warning.ToString() + "\r\n";
                        //Console.WriteLine(warning.ToString());
                    }
                    //htmlValidate.printWarningsToFile();
                }
                else
                    strResult += "This document is a valid html document" + "\r\n";
            }
            catch (ControlException ex)
            {
                strResult += ex.Message + "    " + ex.errorTimeStamp.ToString() + "\r\n";
